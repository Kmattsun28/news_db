...
class FinnhubNewsCollector:
    ...
    # --- 修正：ここから下のキーワードリスト群をすべて削除 ---
    # currency_keywords, usd_sources, eur_sources, jpy_sources,
    # usd_countries, eur_countries, jpy_countries, pair_keywords は不要になりました。

    @staticmethod
    def is_currency_related(article: Dict) -> bool:
        # --- 修正：is_currency_related の実装を書き換え ---
        # 記事のヘッドライン、要約、URLを結合してテキストを作成
        text = (article.get("headline", "") + " " + article.get("summary", "") + " " + article.get("url", "")).lower()
        # utils_scraperのタグ付け関数を呼び出し、タグが1つでもあればTrue（為替関連）と判断
        return len(detect_currency_tags(text)) > 0
    ...
    def get_forex_news(self, limit: int = 50, minutes_back: int = 30) -> List[Dict]:
        ...
        for article in all_news:
            if article.get("url") and article["url"] not in seen_urls:
                ...
                # キーワード判定を修正したis_currency_relatedを使用
    ...
    def convert_to_news_article(self, finnhub_article: Dict) -> Optional[NewsArticle]:
        ...
        try:
            ...
            # --- 修正：タグ付け処理を `detect_currency_tags` に一本化 ---
    ...
    def fetch_and_store_finnhub_news(self, limit: int = 30, minutes_back: int = 30) -> int:
        ...
        try:
            ...
            for article_data in finnhub_news:
                try:
                    # is_currency_related を使った判定は get_forex_news 内で行われているので、ここでは不要
